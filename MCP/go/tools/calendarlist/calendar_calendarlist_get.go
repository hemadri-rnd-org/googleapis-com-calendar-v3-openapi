...
func Calendar_calendarlist_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_calendarlist_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_me_calendarList_calendarId",
		mcp.WithDescription("Returns a calendar from the user's calendar list."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_calendarlist_getHandler(cfg),
	}
}
