...
func Calendar_calendarlist_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_calendarlist_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_users_me_calendarList_calendarId",
		mcp.WithDescription("Updates an existing calendar on the user's calendar list."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_calendarlist_updateHandler(cfg),
	}
}
