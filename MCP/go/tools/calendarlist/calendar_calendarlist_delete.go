...
func Calendar_calendarlist_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_calendarlist_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_users_me_calendarList_calendarId",
		mcp.WithDescription("Removes a calendar from the user's calendar list."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_calendarlist_deleteHandler(cfg),
	}
}
