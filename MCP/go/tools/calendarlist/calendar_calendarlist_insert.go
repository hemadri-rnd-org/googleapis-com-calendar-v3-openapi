...
func Calendar_calendarlist_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_calendarlist_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_users_me_calendarList",
		mcp.WithDescription("Inserts an existing calendar into the user's calendar list."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_calendarlist_insertHandler(cfg),
	}
}
