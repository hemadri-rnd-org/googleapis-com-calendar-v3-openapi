...
func Calendar_events_watchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/calendars/%s/events/watch%s", cfg.BaseURL, calendarId, queryString)
...
func CreateCalendar_events_watchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_calendars_calendarId_events_watch",
		mcp.WithDescription("Watch for changes to Events resources."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_watchHandler(cfg),
	}
}
