...
func Calendar_events_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_events_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_calendars_calendarId_events_eventId",
		mcp.WithDescription("Updates an event."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_updateHandler(cfg),
	}
}
