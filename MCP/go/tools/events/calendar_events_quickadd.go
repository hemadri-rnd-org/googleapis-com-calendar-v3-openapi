...
func Calendar_events_quickaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/calendars/%s/events/quickAdd%s", cfg.BaseURL, calendarId, queryString)
...
func CreateCalendar_events_quickaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_calendars_calendarId_events_quickAdd",
		mcp.WithDescription("Creates an event based on a simple text string."),
		...
		mcp.WithString("text", mcp.Required(), mcp.Description("The text describing the event to be created.")),
		...
		mcp.WithString("sendUpdates", mcp.Description("Guests who should receive notifications about the creation of the new event.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_quickaddHandler(cfg),
	}
}
