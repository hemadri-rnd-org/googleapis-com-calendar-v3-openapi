...
func Calendar_events_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_events_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_calendars_calendarId_events_eventId",
		mcp.WithDescription("Deletes an event."),
		...
		mcp.WithString("sendUpdates", mcp.Description("Guests who should receive notifications about the deletion of the event.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_deleteHandler(cfg),
	}
}
