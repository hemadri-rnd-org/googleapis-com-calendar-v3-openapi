...
func Calendar_events_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_events_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_calendars_calendarId_events_eventId",
		mcp.WithDescription("Updates an event. This method supports patch semantics."),
		...
		mcp.WithString("sendUpdates", mcp.Description("Guests who should receive notifications about the event update (for example, title changes, etc.).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_patchHandler(cfg),
	}
}
