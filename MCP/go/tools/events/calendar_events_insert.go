...
func Calendar_events_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_events_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_calendars_calendarId_events",
		mcp.WithDescription("Creates an event."),
		...
		mcp.WithString("sendUpdates", mcp.Description("Whether to send notifications about the creation of the new event. Note that some emails might still be sent. The default is false.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_events_insertHandler(cfg),
	}
}
