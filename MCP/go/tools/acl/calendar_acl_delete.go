...
func Calendar_acl_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_acl_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_calendars_calendarId_acl_ruleId",
		mcp.WithDescription("Deletes an access control rule."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_acl_deleteHandler(cfg),
	}
}
