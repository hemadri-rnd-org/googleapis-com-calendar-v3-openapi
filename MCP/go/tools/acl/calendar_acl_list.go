...
func Calendar_acl_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_acl_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_calendars_calendarId_acl",
		mcp.WithDescription("Returns the rules in the access control list for the calendar."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_acl_listHandler(cfg),
	}
}
