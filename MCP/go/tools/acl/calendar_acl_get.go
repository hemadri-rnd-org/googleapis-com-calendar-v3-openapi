...
func Calendar_acl_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCalendar_acl_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_calendars_calendarId_acl_ruleId",
		mcp.WithDescription("Returns an access control rule."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Calendar_acl_getHandler(cfg),
	}
}
